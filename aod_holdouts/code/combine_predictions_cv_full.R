...
# This is for the purpose of making predictions on the FULL data
# Cross validation folds are based on the full observed data
...
for (day in days) {
  ...
  lk_df <- readRDS(paste0("./aod_holdouts/cvdata_full/lk_fcv_", day, ".rds")) %>%
    arrange(cmaq_id)
  rf_df <- readRDS(paste0("./aod_holdouts/cvdata_full/rf_fcv_", day, ".rds")) %>%
    arrange(cmaq_id)
...
# Save
saveRDS(df_list, "./aod_holdouts/cvdata_full/fcv_list.rds")
...
# LK has a massive advantage (27 of 31 days)
...
saveRDS(weight_results, "./aod_holdouts/cvdata_full/slfull_weights.rds")
